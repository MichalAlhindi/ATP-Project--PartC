...
public class MyModel extends Observable implements IModel{
    ...
    public int[][] getMaze() {return maze.getMazeArr(); }
    ...
    public int[][] getSolution() {
        ...
        for (int i = 0; i < sol.size(); i++) {
            solutionAsArray[i][0] = ((MazeState) sol.get(i)).getRow();
            solutionAsArray[i][1] = ((MazeState) sol.get(i)).getCol();
        }
...
//        int[] hint = new int[2];
//        hint[0] = ((MazeState) this.hint).getRow();
//        hint[1] = ((MazeState) this.hint).getCol();
//
//        return hint;
//    }
...
//    }
            ...
            Client client = new Client(InetAddress.getLocalHost(), 5401, new IClientStrategy() {
                ...
                public void clientStrategy(InputStream inFromServer,OutputStream outToServer) {
                    try {
                        ...
                        LOG.info("client ask to solve maze with algorithm "+algo);
                        ObjectOutputStream toServer = new ObjectOutputStream(outToServer);
                        ObjectInputStream fromServer = new ObjectInputStream(inFromServer);
                        toServer.flush();
                        maze.setStartPoint(playerRow,playerCol);
    ...
    public void updatePlayerPosition(MovementDirection direction){
        ...
        {
            case DOWNLEFT -> {
                if (maze.getCellValue(playerRow, playerCol - 1) == 0 || maze.getCellValue(playerRow + 1, playerCol) == 0) {
                    tempRow++;
                    tempCol--;
                }
            }
